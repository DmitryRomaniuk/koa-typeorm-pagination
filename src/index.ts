...
> extends Page {
    ...
    toJSON(): Page {
        if (this.indexed) {
            const ids: Array<string | number> = this.content.map((it) => it.id)
            const contentById: { [index: string]: T } = keyBy(
                this.content,
                'id'
            )
            return new IndexedPage(
...
export async function paginate(ctx: Context, next: Function) {
    ...
    const size = parseOptionalIntOrThrow(ctx.query.size) || 20
